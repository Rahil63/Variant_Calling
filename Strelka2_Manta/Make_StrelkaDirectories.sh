...
ls *-t*.bam | \
  awk -F "-t" '{print $1}' | \
    parallel "$STRELKA2/configureStrelkaSomaticWorkflow.py \
      ...
      --callRegions=${PRIMARY} \
      --runDir=VariantDir_${BASENAME}/Strelka2/Somatic \
      --referenceFasta=${HG38} \
      --indelCandidates=VariantDir_${BASENAME}/Manta/Somatic/results/variants/candidateSmallIndels.vcf.gz"
...
ls *-n* | \
  awk -F "-n" '{print $1}' | \
    parallel "$STRELKA2/configureStrelkaGermlineWorkflow.py \
      ...
      --callRegions=${PRIMARY} \
      --runDir=VariantDir_${BASENAME}/Strelka2/Germline/ \
      --referenceFasta=${HG38} \
      --indelCandidates=VariantDir_${BASENAME}/Manta/Germline/results/variants/candidateSmallIndels.vcf.gz"
