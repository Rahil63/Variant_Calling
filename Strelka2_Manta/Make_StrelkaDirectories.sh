...
ls *-t*.bam | \
  awk -F "-t" '{print $1}' | \
    parallel "$STRELKA2/configureStrelkaSomaticWorkflow.py \
      ...
      --callRegions=${PRIMARY} \
      --runDir=Strelka2Dir_{}/Somatic/ \
      --referenceFasta=${HG38} \
      --indelCandidates=./MantaDir_{}/Somatic/results/variants/candidateSmallIndels.vcf.gz"
...
ls *-n* | \
  awk -F "-n" '{print $1}' | \
    parallel "$STRELKA2/configureStrelkaGermlineWorkflow.py \
      ...
      --callRegions=${PRIMARY} \
      --runDir=Strelka2Dir_{}/Germline \
      --referenceFasta=${HG38} \
      --indelCandidates=./MantaDir_{}/Germline/results/variants/candidateSmallIndels.vcf.gz"
