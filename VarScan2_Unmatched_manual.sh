...
mapfile -n 1 < <(head -n 1000 ${BASENAME}_${REGION}_raw.vcf | grep -v '#' | awk NF)
if ((${#MAPFILE[@]}==0)); then
...
egrep -hv "^#" ${BASENAME}_${REGION}_raw.vcf | \
...
if ((${#MAPFILE[@]} > 0)); then
  ...
  $VARSCAN2 fpfilter ${BASENAME}_${REGION}_raw.vcf <(bgzip -c -d -@ 6 ${BASENAME}.bamRC.gz) \
    --output-file ${BASENAME}_${REGION}_fppassed.vcf --filtered-file ${BASENAME}_${REGION}_fpfailed.vcf
  echo ''
